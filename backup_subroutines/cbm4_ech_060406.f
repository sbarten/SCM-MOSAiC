...
C LG-   added the calculation of the available water in liters!
        ...
        H2OLTR=ZPQ(JL,JK)*ZRHOA*1.E-3 ! zpq in g H2O g-1 air and zrhoa 
	     ! in g air cm-3 gives g H2O cm-3. 1 g = 1e-3 kg => 1e-3 liter cm-3
...
C      communication, March, 2002)
       ...
       RMTTERPOH(JL)=2.1E-10   ! mz_lg_20060403+ based on ratio terpinolene/
                               ! humulene of 38/28 minutes lifetime, check table 
       RMTTERPO3(JL)=180.E-17  ! mz_lg_20060403+ based on ratio terpinolene/
...
!      products having 2- and 1- double bonds
       ...
       RSQTERP2BO3(JL)=1170.E-18
       RSQTERP2BNO3(JL)=3.5E-12

       RSQTERP1BOH(JL)=2.9E-12
       RSQTERP1BO3(JL)=1170.E-19
...
!  
!        IF (JK.EQ.NLEV) THEN
!          print *,'cbm4_ech: RNO2H2O',5.6E-6*MIN(1.,100./PBLHGT(JL)),
!      &     RNO2H2O(JL),H2OLTR,ZPQ(JL,JK),ZRHOA
!          IF(NSTEP.EQ.0) THEN
...
C LG-   added the monoterpenes as a group
        ...
        MTTERP0(JL)=ZPM(JL,JK,imtterp) ! mz_lg_20060403+ extra monoterpene
...
! mz_lg-20060330+ added the 2- and 1- double bond oxidation products
        ...
        SQTERP2B0(JL)=ZPM(JL,JK,isqterp2b)
        SQTERP1B0(JL)=ZPM(JL,JK,isqterp1b)
...
C     April 2003, to include the ozonolysis of the sesquiterpenes
     ...
     *      +0.1*RSQTERP2BO3(JL)*SQTERP2B(JL)*O3(JL) ! what is the yield of 2-double bond sesquiterpene-O3 product ?
     *      +0.1*RSQTERP1BO3(JL)*SQTERP1B(JL)*O3(JL) ! what is the yield of 1-double bond sesquiterpene-O3 product ?
...
C LG- and the monoterpenes as a group
        ...
        ZPM(JL,JK,imtterp)=MTTERP(JL) ! mz_lg_20060403+ extra monoterpene

C LG- and the sesquiterpenes as a group

        ZPM(JL,JK,isqterp)=SQTERP(JL)
        ZPM(JL,JK,isqterp2b)=SQTERP2B(JL) ! mz_lg_20060330+
        ZPM(JL,JK,isqterp1b)=SQTERP1B(JL)
...
C         for the reaction rates as a function of the longitude)
          ...
          NTRAC_OUT=7
	  NREACTION(io3)=18  ! mz_lg_20060316+ modified
	  NREACTION(ich2o)=32
	  NREACTION(iisop)=4
          NREACTION(ino_pr)=12
	  ...
	  NREACTION(ioh_pr)=51 ! mz_lg_20060403+ 
...
C LG-     isoprene
	  ...
	  WRITE(NUNREAC,'(100i5)')NRISOPOH,NRISOPO3,NRISOPNO3,
     *          NRISOPNO2

C LG-     NO

	  WRITE(NUNREAC,'(100i5)')JNO2,JBNO3,NRISOPNO2,NRHO2NO,
     *          NRMO2NO,NRC23NO,NRXO2NO,NRMC23NO,NRNONO3,NRXO2NNO,
     *          NRBXO2NNO,NRNOO3

C LG-     NO2

	  WRITE(NUNREAC,'(100i5)')NRHO2NO,NRMO2NO,NRC23NO,NRXO2NO,
...
C LG-     H2O2
	  ...
	  WRITE(NUNREAC,'(100i5)')NRHO2HO2,NRRCHOHO2H,NRHPOH,JH2O2

C LG-     CH3O2H

	  WRITE(NUNREAC,'(100i5)')NRMO2HO2,NROHPCAT,NROHPFRM,JMEPE

C LG-     OH to other outputfile!

	  WRITE(NUNREAC2,'(100i5)')JHNO3,JO3D,JH2O2,JMEPE,
     *      NRISPDO3,NROLEO3,NRISOPO3,NRMTHCO3,NRMVKO3,NRMPANO3,
     *      NRMATERPO3,NRMBTERPO3,NRMTTERPO3,NRSQTERPO3,JHONO,NR65,NRHNO4OH,NRHO2OH,
     ...
     *      NRMATERPOH,NRMBTERPOH,NRMTTERPOH,NRSQTERPOH,NRNOOH
...
C LG-           and destruction terms
     ...
     *          -ZPM(JL,JK,io3)*RMTTERPO3(JL)*ZPM(JL,JK,imtterp)!,'O3+other terp'
...
C LG-    production terms of OH
     ...
     *          ZPM(JL,JK,imtterp)*1.0*RMTTERPO3(JL)*ZPM(JL,JK,io3),
...
C LG-    and destruction terms
     ...
     *         -ZPM(JL,JK,ioh)*RMTTERPOH(JL)*ZPM(JL,JK,imtterp),
...
C LG-    production terms of OH
     ...
     *          ZPM(JL,JK,imaterp)*0.76*RMATERPO3(JL)*ZPM(JL,JK,io3),
     *          ZPM(JL,JK,imbterp)*0.33*RMBTERPO3(JL)*ZPM(JL,JK,io3),
     *          ZPM(JL,JK,imtterp)*1.00*RMTTERPO3(JL)*ZPM(JL,JK,io3),
     *          ZPM(JL,JK,isqterp)*0.22*RSQTERPO3(JL)*ZPM(JL,JK,io3),
     *          RJHONO(JL)*ZPM(JL,JK,ihono),
...
C LG-    and destruction terms
     ...
     *         -ZPMLOC(JL,JK,ioh)*RMTTERPOH(JL)*ZPM(JL,JK,imtterp),
