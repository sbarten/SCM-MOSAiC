...
C LG- assigning filename of file with vegetation data
...
C     this is then done for the right coordinates 
  ...
  999 CONTINUE
      ...
      WRITE(NUNMDFL,'(2a,2i4)')' Determining the land cover and its ',
...
C      ecosystem name and a short description
     ...
     &    dir(1:iip-1)//'input/veg/Olson_73.dat',STATUS='OLD')
       ...
       DO I=0,MAXCLASS
        READ (11,'(A,1X,A)') NUMBER,OLSNAME(I)
       ENDDO
...
C     stomatal exchange by the physiological model by Ronda et al., 2001
      ...
      IF (.NOT.LAGRIAN.AND.IRESET_VEG.EQ.1) GOTO 1001
...
C     calculations, the canopy height is related the number of layers in 
...
C LG-   defining the height for the equidistant coord. system
 ...
 128   CONTINUE
       IF (HC.GT.Z(NLEV+1)) HC=ZHP(NLEV+1)
  ...
  110 CONTINUE 
     ...
     *     OLSVEG(ZILON,ZILAT),OLSON(1:IP-1)
       IF (OLSVEG(ZILON,ZILAT).GT.0) THEN
        ...
        IF (NLEVVEG.GT.0)
     *   WRITE(*,'(1a,i3)')' The number of vegetation layers: ',NLEVVEG
     ...
     *    ' OVOCs emission factor [ug C m-2 hr-1]: ',EMISFACT_VOC(3)  
        ...
        IF (LAGS) 
     *    WRITE(*,'(1a,I6.1)')
     *    ' C3/C4 Plant type (1=C3, 2=C4, 3=coniferous trees): ',PC3C4TYPE    
        IF (.NOT.LAGRIAN) THEN
...
C     the radiation regime within the canopy
     ...
     &              LAGRIAN,LBIOSPH,LVOCEMIS,ZHP,Z,HC,HCCORR,LAD)
...
c ----------------------------------------------------
     ...
     &     ZDZ,ZH(21)
      ...
      LOGICAL LAGRIAN,LBIOSPH,LVOCEMIS,LDTHICK
...
C     mode, the height levels are not defined and thus are calculated here!
      ...
      IF (.NOT.LBIOSPH.AND.LVOCEMIS) THEN
