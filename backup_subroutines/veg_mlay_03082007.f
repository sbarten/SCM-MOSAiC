...
C     included.
      ...
      INTEGER NNFIELD
      PARAMETER (NNFIELD=7)     ! number of input fields for interpolation
     ...
     &     DTIME_MIXING(NLEVV_ML),RLEAFW(NLON,NTRACT),
     &     CH3OH_MLAY(NLEVV_ML)  ! ESS_lg_20070802+ 
...
! mz_lg_20060628+ cloud cover tile index
      INTEGER NT
...
C     and the number of species forming NOX
      ...
      PARAMETER (HCMIN=0.5,PTMST_MIN=10.,NNOXSPEC=5,STOMBLOCK=1.)

      CALL RESETI(ISUM,0)       ! default setting to zero
      CALL RESETR(RLEAFW,NLON*NTRACT,1.E10) ! initializing the wetleaf resistance

      CALL RESETR(FLUXCTOP,NLON*NTRACT,0.) ! mz_lg_20051212+
      CALL RESETR(SURFFLUX,NLON*NTRAC,0.)  ! mz_lg_20051212+
...
C     LG- 
      ...
      DTIME_MIXING(1)=MAX(2.*PTMST,7200.) ! minimal two times the timestep
      DTIME_MIXING(NLEVV_ML)=MAX(2.*PTMST,7200.) ! 7200.

! mz_lg_20060429+ 
      IF (NSTEP.EQ.0) LRESET_RS=.FALSE. ! resetting initialized surface uptake resistances
...
! mz_lg_20060628+ modified
         FIELDIN(JK,2)=0.
         FIELDIN(JK,3)=0.
         FIELDIN(JK,4)=0.
         DO NT=1,NCCTILES
           FIELDIN(JK,2)=FIELDIN(JK,2)+
     &        FEMISOP*FCCTILE(NT)*ISOPEM(NT,JJK)*1.E3*(1./12.)*(1./5.)*AVO
           ISOPSUM=ISOPSUM+FEMISOP*FCCTILE(NT)*ISOPEM(NT,JJK)*
     &        1.E3*(1./12.)*(1./5.)*AVO

C     LG-  added the monoterpene emissions, C10

           FIELDIN(JK,3)=FIELDIN(JK,3)+
     &        FCCTILE(NT)*MTERPEM(NT,JJK)*1.E3*(1./12.)*(1./10.)*AVO

C     LG-  added the sesquiterpene emissions, C15, these are mimiced using the 
C     OVOC's emissions fluxes

           FIELDIN(JK,4)=FIELDIN(JK,4)+
     &        FCCTILE(NT)*OVOCEM(NT,JJK)*1.E3*(1./12.)*(1./15.)*AVO

! ESS_lg_20070802+ other VOC emissions
!          CH3OH

           FIELDIN(JK,5)=FIELDIN(JK,5)+
     &        FCCTILE(NT)*CH3OHEM(NT,JJK)*1.E3*(1./12.)*(1./1.)*AVO
...
! mz_lg_20060628-
         ...
         ISUM(2)=1  ! summing the emissions 
         ISUM(3)=1  ! summing the emissions 
         ISUM(4)=1  ! summing the emissions 
...
C     later done for the other trace gases, using the O3 interpolated field 
         ...
         IF (LAGS) THEN
            FIELDIN(JK,7)=RCOX_AGSML(JJK,io3)
         ELSE
            FIELDIN(JK,7)=RCOX_ECHML(JJK,io3)
         ENDIF
      ENDDO
...
C   LG-  to warn in case that the interpolation didn't work properly
         ...
         IF (JK.EQ.NLEVV_ML.AND.
     &      (.NOT.LAGRIAN.AND.LADSUM.LT.0.99.OR.  ! mz_lg_20051212+
     &        LAGRIAN.AND.HC.GT.HCMIN.AND.LADSUM.LT.0.99)) THEN
            WRITE(*,'(1a)')'veg_mlay: problem with interpolation'
            WRITE(*,'(1a)')'The summated LAD < 0.99: STOP CALLED in veg_mlay'
            WRITE(*,'(1a,f4.2)')'summated LAD: ',LADSUM
            STOP
         ENDIF
         ...
         CH3OH_MLAY(JK)=FIELDOUT(JK,5)  ! ESS_lg_20070802+ added, note the shift of index of other parameters
         U_MLAY(JK)=FIELDOUT(JK,6)
         RCOX_MLAY(JK,io3)=FIELDOUT(JK,7)
...
C     calculated in the subroutine VOCEMIS.f 
         ...
         DO JL=1,NLON
            IF ((VEGFRAC(JL)).GT.0.) THEN
               DO JK=1,NLEVV_ML
                  EMISFLUX_VEG(JK,iisop)=ISOP_MLAY(JK)/(VEGFRAC(JL)+WSFRAC(JL))

! mz_lg_20060313+ for tests of the sensitivity of the isoprene chemistry to the
!                 turbulent mixing, the read-in isoprene emission (isopem)
!                 can be applied to constrain the calculations. In order to
!                 ensure exactly the same emissions for a change in meteorology
!                 then also the varying parameter wsfrac should be removed from
!                 this calculation, also implying that we only have emissions
!                 from the dry vegetation. NOTE THAT THEN ALSO THE IF THEN 
!                 STATEMENT ABOVE MUST BE MODIFIED!

!                  EMISFLUX_VEG(JK,iisop)=ISOP_MLAY(JK)/(VEGFRAC(JL))

                  EMISFLUX_VEG(JK,imaterp)=FEMMATERP*MTERP_MLAY(JK)/
     &                 (VEGFRAC(JL)+WSFRAC(JL))
                  EMISFLUX_VEG(JK,imbterp)=FEMMBTERP*MTERP_MLAY(JK)/
     &                 (VEGFRAC(JL)+WSFRAC(JL))
                  EMISFLUX_VEG(JK,imtterp)=FEMMTTERP*MTERP_MLAY(JK)/
     &                 (VEGFRAC(JL)+WSFRAC(JL))
                  EMISFLUX_VEG(JK,isqterp)=FEMOVOC*OVOC_MLAY(JK)/
     &                 (VEGFRAC(JL)+WSFRAC(JL))

C     LG-         CH3OH canopy flux, simply scaled with the LAD profiles
                  IF (CCOMP(ich3oh).EQ.0.)
     &               EMISFLUX_VEG(JK,ich3oh)=LAD_MLAY(JK)*FEMCH3OH*CH3OHMOLEC/
     &                   (VEGFRAC(JL)+WSFRAC(JL))
                  ! ESS_lg_200708002+ added 
                  IF (CH3OH_MLAY(JK).GT.0) 
     &               EMISFLUX_VEG(JK,ich3oh)=FEMCH3OH*CH3OH_MLAY(JK)/
     &                 (VEGFRAC(JL)+WSFRAC(JL))
                  ! ESS_lg_200708002-

C     LG-         Acetone canopy flux
                  IF (CCOMP(iacet).EQ.0.)
     &               EMISFLUX_VEG(JK,iacet)=LAD_MLAY(JK)*FEMACET*ACETMOLEC/
     &                   (VEGFRAC(JL)+WSFRAC(JL))

! mz_lg_20070302+ included the CH4 emission flux by tropical vegetation, the
!                 typical emission flux for tropical forest is 5 mg CH4 m-2 day,
!                 recalculating this to g CH4 m-2 s-1 gives 5.79e-8, divided by
!                 16 and multiplied with avogadro gives then molecules CH4 m-2 s-1 

                  EMISFLUX_VEG(JK,ich4)=0.25*LAD_MLAY(JK)*(LAI/5.)*  ! rescaled with LAI of 10 used by Sander in TM5 simlations
     &                (1.+4.*JNO2_VEG(JK)/5.E-3)*(5.79E-8/16)*AVO     ! the term
                        ! 0.2 is a rescaling factor to arrive at an average flux of 5mg CH4 m-2 day-1 for Gabriel simulations
         ...
         DO JT=1,NTRACT
            DO JL=1,NLON
               DO JK=1,NLEVV_ML
                  JJK=JK+NLEV
                  QV=1.E5
! mz_lg_20060104+ resetting the stomatal exchange to 1e5 whenever an
!     an vegetation emission flux is defined to avoid the combined calculation
!     of stomatal removal as well as emissions (mostly likely via stomata)

                  IF (EMISFLUX_VEG(JK,JT).GT.0.) RCOX_MLAY(JK,JT)=1.E5 

C     LG-   19092003, redefining the HNO3 surface uptake resistance
...
C     LG-   19092003, and that of H2O2
 		      ...
 		      RCUT(JT)=1.
                      RSOIL(JT)=1.
                      IF (NSTEP.EQ.0) THEN
                        PRINT *,'veg_mlay, be carefull: H2O2 SURF. RESISTANCES. set to 1 s m-1'
 		        PRINT *,'ENTER to continue' 
 			READ (*,*)
 		      ENDIF 
  		    ENDIF                    

! C     LG-   19092003, Gabriel: and that of CH2O
! 
!                     IF (JT.EQ.ich2o) THEN 
!                       RMES(JT)=1.
!  		      RWS(JT)=1.
...
!                       RSOIL(JT)=1.
!                       IF (NSTEP.EQ.0) THEN
!                         PRINT *,'veg_mlay, be carefull: CH2O SURF. RESISTANCES. set to 1 s m-1'
!  		        PRINT *,'ENTER to continue' 
!  			READ (*,*)
!  		      ENDIF 
!   		    ENDIF        

! C     LG-   19092003, and that of CH3OH and acetone, MVK and methacroleine 
! C                   at nighttime
! 
!                     IF (RG.LT.1) THEN
!                       LRESET_RS=.TRUE.
! 		      IF (JT.EQ.ich3oh.OR.JT.EQ.iacet) THEN
!                         RCUT(JT)=RWS(JT)
!                         IF (NSTEP.EQ.0) THEN
!                           WRITE(*,'(2a)')' veg_mlay, be carefull: ',
!      &                    'nighttime CH3OH and acetone cut. resistance set to RWS'
!  		          PRINT *,'ENTER to continue' 
!  			  READ (*,*)
!  		        ENDIF 
!  		      ENDIF  
! 		      IF (JT.EQ.imvk.OR.JT.EQ.imethac) THEN
!  		        RWS(JT)=1.
!                         RCUT(JT)=1.
!                         IF (NSTEP.EQ.0) THEN
!                           WRITE(*,'(2a)')' veg_mlay, be carefull: ',
!      &                     'nighttime MVK and METHAC cut. resistance set to RWS'
!  		          PRINT *,'ENTER to continue' 
!  			  READ (*,*)
!  		        ENDIF 
!  		      ENDIF                    
! 
!                     ! mz_lg_20060429+ in order to reset the resistances to
!                     !  their original values as calculated at the start of 
!                     !  the simulation, whenever the condition for which one
!                     !  tracer resistances is reset is not non fullfilled anymore,
!                     !  the subroutine vd_rs is called for JT=1 and when LRESET_RS
!                     !  is .FALSE.
! 
!                     ELSEIF (RG.GE.1.AND.JT.EQ.1.AND.LRESET_RS) THEN
!                       CALL VD_RS(NSTEP)
!                       LRESET_RS=.FALSE.
!                     ENDIF
...
! C     LG-   19092003, redefining the isoprene surface uptake resistance
! 
!                   IF (JT.EQ.iisop) THEN
! 		    IF (ISOP_MLAY(JK).EQ.0.) THEN
!                       IF (NSTEP.EQ.0) 
!      &                  print *,'veg_mlay, be carefull: ISOPRENE CUT. RESISTANCES. set to 1000. s m-1'
! 		      RCUT(JT)=1000.
! 		    ELSE
! 		      RCUT(JT)=1.E5
! 		    ENDIF
...
C     for canopy layers (see also ddv.f)
                  ...
                  RBML(JK,JT)=DIFFRB(JT)/DIFFRB(iso2)*303.*(0.07/U_MLAY(JK))**0.5
...
C     &  (-1.-(PM(1,NLEV+JK,JT)-CCOMP(JT))/CCOMP(JT))**7
     ...
     &                          CCOMP(JT)/(PRHOA(1,NLEV)/(ZMAIR*1.E9/AVO)),' ppbv'
                           ...
                           READ(*,*)
                        ENDIF
! mz_lg_20060224+ note the modifications in the calculation of the exchanges
!     considering the compensation point throughout veg_mlay. f                        

                        IF (JT.LE.NTRAC) THEN
                           IF (PM(1,NLEV+JK,JT).LT.CCOMP(JT)) THEN
                              RCOX_MLAY(JK,JT)=0.  ! mz_lg_20070705+
                              RLEAF(JL,JT)=-RCOX_MLAY(JK,JT)   !+(1.E5-RCOX_MLAY(JK,JT))*
    ! &                             (-1.-(PM(1,NLEV+JK,JT)-CCOMP(JT))/CCOMP(JT))**7 
                           ELSEIF (PM(1,NLEV+JK,JT).GT.CCOMP(JT)) THEN  !.AND.
...
C     LG-      short lived species
    ...
    ! &                             (-1.-(PMLOC(1,NLEV+JK,JT-NTRAC)-CCOMP(JT))/CCOMP(JT))**7 
                           ELSEIF (PMLOC(1,NLEV+JK,JT-NTRAC).GT.CCOMP(JT)) THEN !.AND.
...
C     Otherwise the "bulk" leaf resistance is being used
                     ...
                     IF (LRCO_ML) THEN
                        IF (JT.LE.NTRAC) THEN
                           IF (PM(1,NLEV+JK,JT).LT.CCOMP(JT)) THEN
                              RLEAF(JL,JT)=-(RBML(JK,JT)+RCOX_MLAY(JK,JT))

C     LG- 112003, modified by introducing the calculation of the leaf resistance
C     for the wet vegetation including the possibility to study the role of the
C     stomatal blocking for the exchanges     

                              RLEAFW(JL,JT)=-(RBML(JK,JT)+RCOX_MLAY(JK,JT))/
     &                          MAX(1.E-10,(1.-STOMBLOCK))
                           ELSEIF (PM(1,NLEV+JK,JT).GT.CCOMP(JT)) THEN
                              RLEAF(JL,JT)=
     &                         (1./((2./(RBML(JK,JT)+RCUT(JT)))+
     &                              (1./(RBML(JK,JT)+RCOX_MLAY(JK,JT)+RMES(JT)))))
...
C     stomatal blocking for the exchanges                  
                              ...
                              RLEAFW(JL,JT)=
     &                         (1./((2./(RBML(JK,JT)+RWS(JT)))+
     &                              (1./(RBML(JK,JT)+RWS(JT)))*STOMBLOCK+
     &                              (1./(RBML(JK,JT)+RCOX_MLAY(JK,JT)+RMES(JT)))*(1.-STOMBLOCK)))
                           ENDIF
                        ELSE
                           IF (PMLOC(1,NLEV+JK,JT-NTRAC).LT.CCOMP(JT)) THEN
                              RLEAF(JL,JT)=-(RBML(JK,JT)+RCOX_MLAY(JK,JT))
...
C     the reference height to the soil surface is also considered.
                     ...
                     IF (RLEAF(JL,JT).GT.0.AND.LAD_MLAY(JK).GT.0.) THEN    ! dry vegetation
                        RSVEG=1./((1./(RSOIL(JT)+RAHCAN(JL)*PZ(JL,JK+NLEV)/CANHEIGHT))+
     &                            (1./(RLEAF(JL,JT)/AMAX1(1E-5,LAD_MLAY(JK)*LAI))))
                     ELSE
                        RSVEG=RSOIL(JT)+RAHCAN(JL)*PZ(JL,JK+NLEV)/CANHEIGHT
                     ENDIF
                     IF (RLEAFW(JL,JT).GT.0.AND.LAD_MLAY(JK).GT.0.) THEN   ! wet vegetation
                        RSWET=1./((1./(RWS(JT)+RAHCAN(JL)*PZ(JL,JK+NLEV)/CANHEIGHT))+
     &                            (1./(RLEAFW(JL,JT)/AMAX1(1E-5,LAD_MLAY(JK)*LAI))))
                     ELSE
		        RSWET=RWS(JT)+RAHCAN(JL)*PZ(JL,JK+NLEV)/CANHEIGHT
                     ENDIF
                  ELSE
...
C     LG- calculation of the crown-layer uptake resistance by simply scaling
C     with the amount of biomass in the layer, only when the leaf uptake 
C     resistances are positive. 

                     IF (RLEAF(JL,JT).GT.0.AND.LAD_MLAY(JK).GT.0.) THEN   ! dry vegetation
                        RSVEG=RLEAF(JL,JT)/AMAX1(1E-5,LAD_MLAY(JK)*LAI)
                     ELSE
		        RSVEG=1.E5
		     ENDIF

                     IF (RLEAFW(JL,JT).GT.0.AND.LAD_MLAY(JK).GT.0.) THEN  ! wet vegetation
                        RSWET=RLEAFW(JL,JT)/AMAX1(1E-5,LAD_MLAY(JK)*LAI)
		     ELSE
		        RSWET=1.E5
		     ENDIF
                  ENDIF

! mz_lg_20051212+ modified to avoid dividing zero for lagragian simulations

                  IF (RSVEG.GT.0.AND.RSWET.GT.0..AND.
     &               (VEGFRAC(JL)+WSFRAC(JL)).GT.0.) THEN
                     RTOT=1./(
     &                    (VEGFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSVEG))+
     &                    (WSFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSWET)) )
                  ELSEIF (RSVEG.GT.0.AND.RSWET.LT.0..AND.
     &                   (VEGFRAC(JL)+WSFRAC(JL)).GT.0.) THEN
                     RTOT=1./
     &                    ((VEGFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSVEG)))
                  ELSEIF (RSVEG.LT.0.AND.RSWET.GT.0..AND.
     &                   (VEGFRAC(JL)+WSFRAC(JL)).GT.0.) THEN
                     RTOT=1./
     &                    ((WSFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSWET)))
                  ELSE
...
C     emission velocity (-1/RLEAF), and the total area of stomata to
...
C                   IF (RLEAF(JL,JT).LT.0..AND.
C      &               (VEGFRAC(JL)+WSFRAC(JL)).GT.0.) THEN  ! mz_lg_20051212+
C                      EMISFLUX_VEG(JK,JT)=EMISFLUX_VEG(JK,JT)+	! Adding to already def. flux.
C      &                    1.E4*SAI*LAD_MLAY(JK)*LAI*            ! stomatal area index * LAI
C      &                    (VEGFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(-100./RLEAF(JL,JT))*
C      &                    (CCOMP(JT)-PM(1,NLEV+JK,JT)) !molecules m-2 s-1
C                   ENDIF

! mz_lg_20060224+ modified calculation of exchanges flux not including anymore
!                 the SAI. The flux in molecules m-2 s-1 is simply calculated
!                 from the concentration gradient in molecules cm-3 and the 
!                 exchange velocity in cm s-1 (100./rsleaf), so simply  
!                 multiplying these terms, *1e14 to arrive at molecules m-2 s-1 
!                 and correcting for LAI

                  IF (RLEAF(JL,JT).LT.0..AND.
     &               (VEGFRAC(JL)+WSFRAC(JL)).GT.0.) THEN  ! mz_lg_20051212+
                     EMISFLUX_VEG(JK,JT)=EMISFLUX_VEG(JK,JT)+	! Adding to already def. flux.
     &                     1.E4*LAD_MLAY(JK)*LAI*               ! corrected for total biomass
     &                    (VEGFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(-100./RLEAF(JL,JT))*
     &                    (CCOMP(JT)-PM(1,NLEV+JK,JT))          ! molecules m-2 s-1

c                  if (jt.eq.ich3oh) then 
c		   print *,'ch3oh, ccomp ',jt,jk,ccomp(jt),clsun_voc,1e-15*emisflux_veg(jk,jt)
c                   if (jk.eq.2) read (*,*)
c		  endif
...
C     LG- calculation of the emissions within the vegetation layers
...
!                   KH(1)=MAX(KHMIN,4.*DZ/RAHVEG(JL))  ! Gabriel sensitivity analysis.
...
C     LG-     112003, old formulation based on using rahcan.
...
C     determines the minimum timestep of the subtimestep scale calculations
                  ...
                  IF (IEM_TURB(JT).EQ.1.OR.IDD_TURB(JT).EQ.1) THEN
                     PTMST_SUB=INT(MAX(MIN(PTMST,PTMST_MIN),
     &                   MIN(PTMST,0.1*MIN(TS_DD,TS_TURB)))) 
                  ELSE
                     PTMST_SUB=INT(PTMST)
                  ENDIF

                  NSTEP_SUB=MAX(1,INT(PTMST/PTMST_SUB))
                  PTMST_SUB=INT(PTMST/NSTEP_SUB)
...
C     LG-  CH3OH and acetone

         CRF(ich3oh)=0.
         IF (EMISFLUX_VEG(NLEVV_ML,ich3oh).GT.0.)
     &        CRF(ich3oh)=1.E11*1.E4*FLUXCTOP(JL,ich3oh)/
     &        (EMISFLUX_VEG(1,ich3oh)+EMISFLUX_VEG(2,ich3oh))

         CRF(iacet)=0.
         IF (EMISFLUX_VEG(NLEVV_ML,iacet).GT.0.)
     &        CRF(iacet)=1.E11*1.E4*FLUXCTOP(JL,iacet)/
     &        (EMISFLUX_VEG(1,iacet)+EMISFLUX_VEG(2,iacet))

C     LG-  monoterpenes and sesquiterpenes

         CRF(imaterp)=0.
         IF (EMISFLUX_VEG(NLEVV_ML,imaterp).GT.0.)
     &        CRF(imaterp)=1.E11*1.E4*FLUXCTOP(JL,imaterp)/
     &        (EMISFLUX_VEG(1,imaterp)+EMISFLUX_VEG(2,imaterp))

         CRF(imbterp)=0.
         IF (EMISFLUX_VEG(NLEVV_ML,imbterp).GT.0.)
     &        CRF(imbterp)=1.E11*1.E4*FLUXCTOP(JL,imbterp)/
     &        (EMISFLUX_VEG(1,imbterp)+EMISFLUX_VEG(2,imbterp))

         CRF(imtterp)=0.
         IF (EMISFLUX_VEG(NLEVV_ML,imtterp).GT.0.)
     &        CRF(imtterp)=1.E11*1.E4*FLUXCTOP(JL,imtterp)/
     &        (EMISFLUX_VEG(1,imtterp)+EMISFLUX_VEG(2,imtterp))

         CRF(isqterp)=0.
         IF (EMISFLUX_VEG(NLEVV_ML,isqterp).GT.0.)
     &        CRF(isqterp)=1.E11*1.E4*FLUXCTOP(JL,isqterp)/
     &        (EMISFLUX_VEG(1,isqterp)+EMISFLUX_VEG(2,isqterp))
...
C     LG- writing of canopy reduction factor and other parameters to output file
     ...
     *           'NOEMIS','CRF(NO)','CRF(NO2)','CRF(NOx)','FLUXCTOP(rad)',
     *           'EMFLXNOX(1)','EMFLXNOX(2)'
         ELSE
            WRITE(NUNMVLAY,'(a10,a15,102a15)')'nstep','time',
     *           'u*[ms-1]','RAHVEG','RAHCAN',
     *           'RBVEG(O3)','RB(1,O3)/LAI','RB(2,O3)/LAI',
     ...
     *           'VdH2O2(1)','VdH2O2(2)','VdCH3OH(1)','VdCH3OH(2)',
     *           'VdACET(1)','VdACET(2)','RCO_MLAY(1)','RCO_MLAY(2)',
     *           'NOx-veg','NOx-surf','NO2-veg','NO2-surf',
     *           'O3-veg','O3-surf','C5H8-veg','C5H8-surf',
     *           'OH-veg','OH-surf','DC(NOx)','DC(O3)','FLUXVEG(O3)','FLUXVEG(NO)',
     *           'FLUXCTOP(O3)','FLUX(O3)','FLUXCTOP(NO)','FLUX(NO)',
     *           'FLUXCTOP(NO2)','FLUX(NO2)','FLUXCTOP(NOx)','FLUX(NOx)',
     *           'NOEMIS','CRF(NO)','CRF(NO2)','CRF(NOx)',
     *           'FLUXCTOP(isop)','FLUX(isop)','ISOPEMIS','CRF(isop)',
     *           'FLUXCTOP(rad)','EMFLXNOX(1)','EMFLXNOX(2)', 
     *           'FLUXCTOP(CO2)','FLUX(CO2)','CO2EMIS','CRF(CO2)',
     *           'FLUXCTOP(NH3)','FLUX(NH3)','NH3EMIS','CRF(NH3)',
     *           'FLUXCTOP(H2O2)','FLUX(H2O2)','FLUXCTOP(MHP)','FLUX(MHP)',
     *           'FLUXCTOP(HMHP)','FLUX(HMHP)',
     *           'FLXCTOP(CH3OH)','FLUX(CH3OH)','CH3OHEMIS','CRF(CH3OH)',
     *           'FLUXCTOP(ACET)','FLUX(ACET)','ACETEMIS','CRF(ACET)',
     *           'FLUXCTOP(MATP)','FLUX(MATERP)','MATERPEMIS','CRF(MATERP)',
     *           'FLUXCTOP(MTTP)','FLUX(MTTERP)','MTTERPEMIS','CRF(MTTERP)',
     *           'FLUXCTOP(CH4) ','FLUX(CH4)   ','CH4EMIS(1)','CH4EMIS(2)'
         ENDIF
      ENDIF
     ...
     *              1.e-4*1.e-11*EMISFLUX_VEG(2,ino2_pr)
            ELSE
               WRITE(NUNMVLAY,
     *              '(1x,i9.9,1x,a14,35f15.4,23e15.4,3f15.4,3e15.4,f15.4,6e15.4,f15.4,
     *              3e15.4,f15.4,6e15.4,3e15.4,f15.4,3e15.4,f15.4,3e15.4,f15.4,3e15.4,f15.4,
     *              4e15.4)')
     *              NSTEP,LDATLTIME,USTAR(1),RAHVEG(1),RAHCAN(1),RBVEG(1,io3),
     *              RBML(1,io3)/AMAX1(1E-5,LAD_MLAY(1)*LAI),
     *              RBML(2,io3)/AMAX1(1E-5,LAD_MLAY(2)*LAI),KH(1),KH(2),
     ...
     *              VDDEP(1,ih2o2),VDDEP(2,ih2o2),VDDEP(1,ich3oh),VDDEP(2,ich3oh),
     *              VDDEP(1,iacet),VDDEP(2,iacet),
     ...
     *              FLUXCTOP(1,iisop),SURFFLUX(1,iisop),
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,iisop)+EMISFLUX_VEG(1,iisop)),
     *              CRF(iisop),FLUXCTOP(1,irad),1.e-4*1.e-11*EMISFLUX_VEG(1,ino2),
     *              1.e-4*1.e-11*EMISFLUX_VEG(2,ino2),1.e-4*FLUXCTOP(1,ico2),
     *              1.e-4*SURFFLUX(1,ico2),
     *              1.e-4*1.e-15*(EMISFLUX_VEG(2,ico2)+EMISFLUX_VEG(1,ico2)),CRF(ico2),
     *              FLUXCTOP(1,inh3),SURFFLUX(1,inh3),
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,inh3)+EMISFLUX_VEG(1,inh3)),CRF(inh3),
     *              FLUXCTOP(1,ih2o2),SURFFLUX(1,ih2o2),
     *              FLUXCTOP(1,ich3o2h),SURFFLUX(1,ich3o2h),
     *              FLUXCTOP(1,ich2oho2h),SURFFLUX(1,ich2oho2h),
     *              FLUXCTOP(1,ich3oh),SURFFLUX(1,ich3oh),
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,ich3oh)+EMISFLUX_VEG(1,ich3oh)),CRF(ich3oh),
     *              FLUXCTOP(1,iacet),SURFFLUX(1,iacet),
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,iacet)+EMISFLUX_VEG(1,iacet)),CRF(iacet),
     *              FLUXCTOP(1,imaterp),SURFFLUX(1,imaterp),
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,imaterp)+EMISFLUX_VEG(1,imaterp)),CRF(imaterp),
     *              FLUXCTOP(1,imtterp),SURFFLUX(1,imtterp),
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,imtterp)+EMISFLUX_VEG(1,imtterp)),CRF(imtterp),
     *              FLUXCTOP(1,ich4),SURFFLUX(1,ich4),
     *              1.e-4*1.e-11*EMISFLUX_VEG(1,ich4),1.e-4*1.e-11*EMISFLUX_VEG(2,ich4)
            ENDIF
         ELSE
            IF (LAIRCHEM) THEN
               WRITE(NUNMVLAY,'(1x,i9.9,1x,a14,20f12.4,21e12.4,3f12.4,3e12.4)')
     ...
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,iisop)+EMISFLUX_VEG(1,iisop)),CRF(iisop),
     *              FLUXCTOP(1,irad),
     *              1.e-4*1.e-11*EMISFLUX_VEG(1,inox),1.e-4*1.e-11*EMISFLUX_VEG(2,inox),
     *              1.e-4*FLUXCTOP(1,ico2),1.e-4*SURFFLUX(1,ico2),
     *              1.e-4*1.e-15*(EMISFLUX_VEG(2,ico2)+EMISFLUX_VEG(1,ico2)),CRF(ico2),
     *              FLUXCTOP(1,inh3),SURFFLUX(1,inh3),
     *              1.e-4*1.e-11*(EMISFLUX_VEG(2,inh3)+EMISFLUX_VEG(1,inh3)),CRF(inh3),
     *              FLUXCTOP(1,ih2o2),SURFFLUX(1,ih2o2),
