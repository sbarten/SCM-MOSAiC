...
C     included.
     ...
     &     RSVEG,RSWET,KHMIN,UAVG,RAH_TOT,SUMU,
     ...
     &     OVOC_MLAY(NLEVV_ML),U_MLAY(NLEVV_ML),FIELDIN(NLEVV,10),
     &     FIELDOUT(NLEVV_ML,10),TOTWEIGHT(NLEVV_ML),RATIO(NLEVV_ML),
     &     KH(NLEVV_ML),ZMVEG(NLON,NLEVV_ML,NTRACT),ZMVEG_OLD(NLEVV_ML),
     &     ZU(NLEVV_ML+1),EM(NLEVV_ML+1),VDD(NLEVV_ML+1,NTRACT),
     &     RECALC(NLEVT),ZKH(NLEVV_ML+1),RB(NLEVV_ML,NTRACT),
...
C LG- 
      ...
      DTIME_MIXING(1)=MAX(2.*PTMST,1.e5)	! minimal two times the timestep
      DTIME_MIXING(NLEVV_ML)=MAX(2.*PTMST,1.e5)
...
C      later done for the other trace gases, using the O3 interpolated field 
      ...
      CALL INTERPOLATE(CANHEIGHT,NLEV,NLEVV_ML,PZHP,6,
...
C       for canopy layers (see also ddv.f)
	...
	!     difussion properties for SO2 
        RB(JK,JT)=DIFFRB(JT)/DIFFRB(iso2)*180.*(0.07/U(NLEVV_ML+1-JK))**0.5
...
C LG-    19092003, overwritting the leaf resistance of HNO3
         ...
         IF (JT.EQ.ih2o2) THEN
	   RLEAF(JL,JT)=1.
	   print *,'veg_mlay, be carefull: RleafH2O2 set to 1 s m-1'
	 ENDIF
...
C        the reference height to the soil surface is also considered.
     ...
     &     (1./((RB(JK,JT)+RLEAF(JL,JT))/AMAX1(1E-5,LAD_MLAY(JK)*LAI))))
         ELSE
          RSVEG=1./((1./(RSOIL(JT)+RAHCAN(JL)*PZ(JL,JK+NLEV)/CANHEIGHT))+
     &     (1./((-RB(JK,JT)+RLEAF(JL,JT))/AMAX1(1E-5,LAD_MLAY(JK)*LAI))))
         ENDIF
         RSWET=1./((1./(RWS(JT)+RAHCAN(JL)*PZ(JL,JK+NLEV)/CANHEIGHT))+
     &     (1./((RB(JK,JT)+RWS(JT))/AMAX1(1E-5,LAD_MLAY(JK)*LAI))))
...
C LG-    19092003, overwritting the leaf resistance of HNO3
         ...
         IF (JT.EQ.ih2o2) RLEAF(JL,JT)=1.

         IF (RLEAF(JL,JT).GT.0.) THEN
          RSVEG=(RB(JK,JT)+RLEAF(JL,JT))/
     &     AMAX1(1E-5,LAD_MLAY(JK)*LAI)
          RSWET=(RB(JK,JT)+RWS(JT))/
     &     AMAX1(1E-5,LAD_MLAY(JK)*LAI)
         ELSE
          RSVEG=(-RB(JK,JT)+RLEAF(JL,JT))/
     &     AMAX1(1E-5,LAD_MLAY(JK)*LAI)
...
C         IF (JT.EQ.ico2.AND.RG.EQ.0.) THEN
C 	  VDDEP(JK,JT)=1.E-10
C           IF (JK.EQ.NLEVV_ML) 
C      &       print *,'Vd CO2 nighttime value: ',vddep(JK,JT)
C 	ENDIF
...
C          print *,'veg_mlay,writing Kh',JK,JK-NLEV+1,ZKH(JK-NLEV+1),
C      *       ZU(JK-NLEV+1)
...
C LG- writing of canopy reduction factor and other parameters to output file
     ...
     *    'EMFLXNOX(1)','EMFLXNOX(2)'
       ELSE
        WRITE(NUNMVLAY,'(a10,a15,74a15)')'nstep','time',
     *    'u*[ms-1]','RAHVEG','RAHCAN',
     *    'RBVEG(O3)','RB(1,O3)/LAI','RB(2,O3)/LAI',
     ...
     *     RB(1,io3)/AMAX1(1E-5,LAD_MLAY(1)*LAI),
     *     RB(2,io3)/AMAX1(1E-5,LAD_MLAY(2)*LAI),KH(1),KH(2),
