...
C     the reference height to the soil surface is also considered.
                     ...
                     IF (RLEAF(JL,JT).GT.0.) THEN    ! dry vegetation
     ...
     &                            (1./(RLEAF(JL,JT)/AMAX1(1E-5,LAD_MLAY(JK)*LAI))))
                     ...
                     ENDIF
                     IF (RLEAFW(JL,JT).GT.0.) THEN   ! wet vegetation
...
C     resistances are positive. 
		     ...
		     ENDIF
                  ...
                  IF (RSVEG.GT.0.AND.RSWET.GT.0.) THEN
                     RTOT=1./(
     &                    (VEGFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSVEG))+
     &                    (WSFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSWET)) )
                  ELSEIF (RSVEG.GT.0.AND.RSWET.LT.0.) THEN
                     RTOT=1./
     &                    ((VEGFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSVEG)))
                  ELSEIF (RSVEG.LT.0.AND.RSWET.GT.0.) THEN
                     RTOT=1./
     &                    ((WSFRAC(JL)/(VEGFRAC(JL)+WSFRAC(JL)))*(1./(RSWET)))
                  ELSE
		     RTOT=1.E5
...
C                   IF (JT.eq.iisop) THEN
C 		   print *,'isoprene subtimestep',NSTEP_SUB,PTMST_SUB,
C      &                TS_TURB,TS_DD,TS_EM
C                   ENDIF
...
C     LG- writing of canopy reduction factor and other parameters to output file
     ...
     *           'EMFLXNOX(1)','EMFLXNOX(2)'
         ELSE
            WRITE(NUNMVLAY,'(a10,a15,78a15)')'nstep','time',
     *           'u*[ms-1]','RAHVEG','RAHCAN',
     *           'RBVEG(O3)','RB(1,O3)/LAI','RB(2,O3)/LAI',
