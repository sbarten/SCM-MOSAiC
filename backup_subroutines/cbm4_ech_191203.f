...
C LG- added trace gases
     ...
     *     HONO0(NLON),MATERP(NLON),MBTERP(NLON),SQTERP(NLON),
     *     HONO(NLON)
...
C      1.5*10^(-11) cm^3 molecule^(-1) s^(-1) Seinfeld&Pandis (1998)
...
C LG- further modified based on personal communications with Kostas, 23 July
C     2002, see also the change in the term P6!
...
C LG- end       
          ...
          XL5=R65+R75+RMO2HO2(JL)*CH3O2(JL)+
     *         RHO2OH(JL)*OH(JL)+
...
C LG- including HONO production through NO+OH
     ...
     *      +RNOOH(JL)*ZNO(JL)
          ...
          HNO4(JL)=(HNO40(JL)+R75*DT*HO2(JL))/C7
...
C     but HNO3 present in the water/aerosol: 2NO2+H2O -> HONO+HNO3 
          ...
          PHONO=RNOOH(JL)*ZNO(JL)*OH(JL)+RNO2H2O(JL)*ZNO2(JL)
	  XLHONO=RJHONO(JL)*HONO(JL)
...
C LG-   assigning the OH production rate for interpretation 
...
C LG-     ozone
     ...
     *     NRISOPO3,NRISPDO3,NRMVKO3,NRMPANO3,NRH2OOD
...
C LG-     H2O2
	  ...
	  WRITE(NUNREAC,'(100i4)')NRHO2HO2,JH2O2,NRHPOH
...
C LG-     OH to other outputfile!
     ...
     *      NRHO2NO,NRO3HO2,NRMATERPO3,NRMBTERPO3,NRSQTERPO3,JHONO,
     *      NRHNO4OH,NRHO2OH,NRNO2OH,NROHHNO3,NRCOOH,NRO3OH,NRHPOH,
     *      NRFRMOH,NRH2OH,NRETHOH,NROLEOH,NRISOPOH,NRISPDOH,NRMTHCOH,
     *      NRMVKOH,NRPAROH,NRMPANOH,NRCH4OH,NROHPCAT,NROHOH,NRPAROH,
     *      NRMGLYOH,NRISOPOH,NRISPDOH,NRMTHCOH,NRMEKOH,NRMVKOH,NRALD2OH,
     *      NRMPANOH,NRACETOH,NRMATERPOH,NRMBTERPOH,NRSQTERPOH,NRNOOH
...
C LG-    production terms of formaldehyde
     ...
     *      1.33*RMO2MO2(JL)*ZPM(JL,JK,ich3o2)**2,
     *      RC23NO(JL)*ZPM(JL,JK,ic2o3)*ZPM(JL,JK,ino),
     *      2*RC23C23(JL)*ZPM(JL,JK,ic2o3)**2,
...
C LG-    production terms of OH
     ...
     *          RJMEPE(JL)*ZPM(JL,JK,ich3o2h),
     *          ZPM(JL,JK,io3)*0.4*RISPDO3(JL)*ZPM(JL,JK,iisoprd),
     *          ZPM(JL,JK,io3)*0.1*ROLEO3(JL)*ZPM(JL,JK,iole),
     *          ZPM(JL,JK,io3)*0.2*RISOPO3(JL)*ZPM(JL,JK,iisop),
     *          ZPM(JL,JK,io3)*0.1*RMTHCO3(JL)*ZPM(JL,JK,imethac),
     *          ZPM(JL,JK,io3)*0.05*RMVKO3(JL)*ZPM(JL,JK,imvk),
     *          ZPM(JL,JK,io3)*0.04*RMPANO3(JL)*ZPM(JL,JK,impan),
     *          ZPM(JL,JK,iho2)*RHO2NO(JL)*ZPM(JL,JK,ino),
     *          ZPM(JL,JK,io3)*RO3HO2(JL)*ZPM(JL,JK,iho2),
...
C LG-    production terms of formaldehyde
     ...
     *      RJALD2(JL)*ZPM(JL,JK,iald2),
     *      1.33*RMO2MO2(JL)*ZPMLOC(JL,JK,ich3o2)**2,
     *      RC23NO(JL)*ZPMLOC(JL,JK,ic2o3)*ZPMLOC(JL,JK,ino),
     *      2*RC23C23(JL)*ZPMLOC(JL,JK,ic2o3)**2,
...
C LG-    production terms of OH
     ...
     *          ZPM(JL,JK,io3)*0.04*RMPANO3(JL)*ZPM(JL,JK,impan),
     *          ZPMLOC(JL,JK,iho2)*RHO2NO(JL)*ZPMLOC(JL,JK,ino),
     *          ZPM(JL,JK,io3)*RO3HO2(JL)*ZPMLOC(JL,JK,iho2),
     *          ZPM(JL,JK,imaterp)*0.76*RMATERPO3(JL)*ZPM(JL,JK,io3),
     *          ZPM(JL,JK,imbterp)*0.33*RMBTERPO3(JL)*ZPM(JL,JK,io3),
     *          ZPM(JL,JK,isqterp)*0.22*RSQTERPO3(JL)*ZPM(JL,JK,io3),
     *          RJHONO(JL)*ZPM(JL,JK,ihono),
...
C LG-    and destruction terms
     ...
     *         -ZPMLOC(JL,JK,ioh)*RH2OH(JL)*AIR(JL),
